...
def check_and_create_hdfs_file(hdfs_url, hdfs_path, content="", overwrite=False):
    """
    检查HDFS文件是否存在，不存在则创建

    参数:
    hdfs_url (str): HDFS Namenode的URL，例如 http://localhost:50070
    ...
    try:
        ...
        if client.status(hdfs_path, strict=False):
            print(f"文件 {hdfs_path} 已存在")
            return True

        # 文件不存在，创建它
        client.write(hdfs_path, data=content, overwrite=overwrite)
        print(f"文件 {hdfs_path} 已成功创建")
        return True
...
if __name__ == "__main__":
    ...
    database = "gmall"
    HDFS_FILE_PATH=f"/bigdata_warehouse/{database}"


    flag = check_and_create_hdfs_file(hdfs_nn, HDFS_FILE_PATH)
    ...
    if flag:
        ...
        for table in tables:
            ...
            location_url = f"LOCATION '{HDFS_URL}/{HDFS_FILE_PATH}/ods_{table}'\n"
            # 生成Hive DDL文件（单独输出）
            generate_hive_ddl(
                table=table,
                columns=columns,
